...
const MockAPI = {
  ...
  // Charge history.json depuis le repo
  async loadHistory() {
    try {
  ...
  // Données graphique depuis historique réel (semaines)
  getChartData(history, platform) {
    if (!history || !history.length) return null;
    const entries = history
      .filter(h => h.platforms && h.platforms[platform] && !h.platforms[platform].error)
      .slice(-24);
    if (!entries.length) return null;
    return {
      labels:     entries.map(h => {
        const d = new Date(h.date);
        return `${d.getDate()}/${d.getMonth()+1}`;
      }),
      followers:  entries.map(h => h.platforms[platform].followers  || 0),
      engagement: entries.map(h => h.platforms[platform].engagement || 0),
      likes:      entries.map(h => h.platforms[platform].likes      || 0),
    };
  },
