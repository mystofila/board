...
/* ============================================================
   APIFY INTEGRATION — ajout dans app.js
============================================================ */
...
function injectStatusBar() {
  ...
  bar.style.cssText = `
    ...
    font-size:12px; color:var(--text-muted); z-index:80;
    transition:left var(--transition);
  `;
  bar.innerHTML = `
    <div id="status-dot" style="width:7px;height:7px;border-radius:50%;background:var(--text-dim);flex-shrink:0"></div>
    <span id="status-text">En attente de chargement…</span>
    <div style="margin-left:auto;display:flex;gap:8px;align-items:center">
      <span id="status-cache"></span>
      <button class="btn-secondary" style="font-size:11px;padding:4px 10px" onclick="refreshData()">↺ Actualiser</button>
    </div>
  `;
...
function setStatus(type, text, cache='') {
  const dot  = document.getElementById('status-dot');
  const txt  = document.getElementById('status-text');
  const ca   = document.getElementById('status-cache');
  if (!dot) return;
  const colors = { loading:'#F59E0B', ok:'#10B981', error:'#EF4444', mock:'#6366F1' };
  dot.style.background = colors[type] || colors.mock;
  if (txt) txt.textContent = text;
  if (ca)  ca.textContent  = cache;
}

// ── Refresh bouton ───────────────────────────────────────────
async function refreshData() {
  if (!ApifyConfig.token) {
    showToast('⚙️ Configurez votre token Apify dans Paramètres d\'abord');
    navigateTo('settings');
    return;
  }
  setStatus('loading', 'Actualisation en cours…');
  MockAPI.clearCache();
  await MockAPI.loadAll(true);
}

// ── Apify event callbacks ─────────────────────────────────────
MockAPI.on((type, payload) => {
  if (type === 'loading') {
    setStatus('loading', 'Connexion à Apify…');
  }
  else if (type === 'partial') {
    const isCached = payload.cached;
    const hasError = payload.error;
    const plat = payload.platform;
    const icon = { tiktok:'🎵', instagram:'📸', facebook:'👍', linkedin:'💼' }[plat];
    if (hasError) {
      setStatus('mock', `${icon} ${plat} — données simulées (${payload.error.slice(0,40)})`);
    } else {
      setStatus(isCached ? 'mock' : 'ok', `${icon} ${plat} chargé${isCached ? ' (cache)' : ' ✓'}`);
    }
    // Re-render current page stats live
    renderStatCards();
  }
  else if (type === 'done') {
    const token   = ApifyConfig.token;
    const info    = MockAPI.getCacheInfo();
    const anyReal = Object.values(payload).some(p => !p.note?.includes('simulé'));
    const cacheStr = Object.entries(info).filter(([,v])=>v.hasData).map(([k,v])=>`${k}: ${v.age}`).join(' · ');
    setStatus(
      token ? (anyReal ? 'ok' : 'error') : 'mock',
      token ? (anyReal ? 'Données Apify chargées ✓' : 'Apify configuré mais erreurs — données simulées') : 'Mode simulation (token Apify non configuré)',
      cacheStr
    );
    // Full re-render
    renderStatCards();
...
// ── Patch settings page pour ajouter section Apify ───────────
const _origRenderApi = renderApiConnections;
function renderApiConnections() {
  const el = document.getElementById('api-connections');
  if (!el) return;
  ...
  el.innerHTML = `
    <div style="margin-bottom:16px">
      <label class="form-label">Token Apify</label>
      <div style="display:flex;gap:8px;margin-top:6px">
        <input type="password" class="form-input" id="apify-token-input"
          placeholder="apify_api_xxxxxxxxxxxx"
          value="${token}"
          style="flex:1;font-family:var(--font-mono);font-size:12px"
        />
        <button class="btn-primary" onclick="saveApifyToken()">Enregistrer</button>
      </div>
      <div style="font-size:11px;color:var(--text-muted);margin-top:6px">
        Créez un compte gratuit sur <a href="https://apify.com" target="_blank" style="color:var(--accent)">apify.com</a>
        → Console → Integrations → API tokens
      </div>
    </div>

    <div style="margin-bottom:16px;padding:12px;background:var(--bg-base);border-radius:8px;border:1px solid var(--border)">
      <div style="font-size:12px;font-weight:600;margin-bottom:10px;color:var(--text-muted)">État du cache (TTL : 7 jours)</div>
      ${['tiktok','instagram','facebook','linkedin'].map(plat => {
        const i = info[plat];
        const icon = { tiktok:'🎵', instagram:'📸', facebook:'👍', linkedin:'💼' }[plat];
        return `<div class="api-connection">
          <span>${icon} ${plat.charAt(0).toUpperCase()+plat.slice(1)}</span>
          <div style="display:flex;align-items:center;gap:8px">
            <span class="api-status ${i.hasData?'connected':'disconnected'}">${i.hasData ? '● ' + i.age : '○ Pas de cache'}</span>
            <button class="btn-secondary" style="font-size:11px;padding:4px 10px"
              onclick="clearPlatformCache('${plat}')">Vider</button>
          </div>
        </div>`;
      }).join('')}
    </div>
    ...
    <div style="display:flex;gap:8px">
      <button class="btn-primary" onclick="refreshData()" style="flex:1">↺ Actualiser maintenant</button>
      <button class="btn-secondary" onclick="clearAllCache()">Vider tout le cache</button>
    </div>

    <div style="margin-top:12px;font-size:11px;color:var(--text-muted);line-height:1.8">
      <strong>Comptes surveillés :</strong><br>
      🎵 TikTok : <a href="https://www.tiktok.com/@afder.recovery" target="_blank" style="color:var(--accent)">@afder.recovery</a><br>
      📸 Instagram : <a href="https://www.instagram.com/afder.recovery/" target="_blank" style="color:var(--accent)">afder.recovery</a><br>
      👍 Facebook : <a href="https://www.facebook.com/afder.recovery" target="_blank" style="color:var(--accent)">afder.recovery</a> + <a href="https://www.facebook.com/OldTimersRecovery/" target="_blank" style="color:var(--accent)">OldTimersRecovery</a><br>
      💼 LinkedIn : <a href="https://www.linkedin.com/company/afder/" target="_blank" style="color:var(--accent)">company/afder</a>
    </div>
  `;
}
...
function saveApifyToken() {
  ...
  showToast('✅ Token enregistré — actualisation…');
  setTimeout(() => refreshData(), 500);
}

function clearPlatformCache(plat) {
  localStorage.removeItem(`apify_cache_${plat}`);
  renderApiConnections();
  showToast(`🗑️ Cache ${plat} vidé`);
}

function clearAllCache() {
  MockAPI.clearCache();
  renderApiConnections();
  showToast('🗑️ Tout le cache vidé');
}

// ── Surcharge de DOMContentLoaded pour injecter + charger ─────
document.addEventListener('DOMContentLoaded', () => {
  injectStatusBar();
  // Lancer le chargement Apify (ou mock si pas de token)
  setTimeout(() => MockAPI.loadAll(), 200);
});
