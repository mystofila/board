...
function buildFollowersChart(plats) {
  ...
  const weekly = (typeof AppState !== 'undefined') ? AppState.weeklyHistory : [];
  let labels = MONTHS;
  let datasets;

  if (weekly && weekly.length >= 2) {
    // Données réelles depuis history.json
    const allLabels = [...new Set(weekly.map(h => {
      const d = new Date(h.date); return `${d.getDate()}/${d.getMonth()+1}`;
    }))];
    labels = allLabels;
    datasets = entries.map(([key]) => {
      const hist = MockAPI.getChartData(weekly, key);
      const data = hist ? hist.followers : [];
      return { label:MockAPI.LABELS[key], data, borderColor:MockAPI.COLORS[key], backgroundColor:grad(ctx,MockAPI.COLORS[key]), borderWidth:2, pointRadius:3, pointBackgroundColor:MockAPI.COLORS[key], tension:0.4, fill:true };
    });
  } else {
    // Pas encore d'historique — courbe estimée depuis valeur actuelle
    datasets = entries.map(([key,p]) => {
      ...
      return { label:MockAPI.LABELS[key], data, borderColor:MockAPI.COLORS[key], backgroundColor:grad(ctx,MockAPI.COLORS[key]), borderWidth:2, pointRadius:0, tension:0.4, fill:true,
        borderDash: weekly.length === 0 ? [4,3] : [] }; // pointillés = estimation
    });
  }

  ChartReg[id]=new Chart(ctx,{type:'line',data:{labels,datasets},options:{
    ...baseOpts,
    plugins:{...baseOpts.plugins, subtitle:{
      display: !weekly || weekly.length < 2,
      text: 'Estimation — données réelles disponibles après la 1ère collecte Apify',
      color:'rgba(248,250,252,0.35)', font:{size:10,style:'italic'}, padding:{bottom:8},
    }},
  }});
...
function buildEngagementChart(plats) {
  ...
  if (weekly && weekly.length >= 2) {
    labels = weekly.map(h => { const d=new Date(h.date); return `${d.getDate()}/${d.getMonth()+1}`; });
    datasets = entries.map(([key]) => {
      const hist = MockAPI.getChartData(weekly, key);
      return { label:MockAPI.LABELS[key], data:hist?hist.engagement:[], borderColor:MockAPI.COLORS[key], backgroundColor:'transparent', borderWidth:2, pointRadius:3, pointBackgroundColor:MockAPI.COLORS[key], tension:0.4 };
    });
  } else {
    datasets = entries.map(([key,p])=>{
      ...
      data[11]=base;
      return {label:MockAPI.LABELS[key],data,borderColor:MockAPI.COLORS[key],backgroundColor:'transparent',borderWidth:2,pointRadius:0,tension:0.4,borderDash:[4,3]};
    });
  }

  ChartReg[id]=new Chart(el.getContext('2d'),{type:'line',data:{labels,datasets},
